...
extension TeamGameStats: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        guard let fastBreakPoints = Int(try container.decode(String.self, forKey: .fastBreakPoints)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.fastBreakPoints], debugDescription: "Value for \"\(RootKeys.fastBreakPoints.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let pointsInPaint = Int(try container.decode(String.self, forKey: .pointsInPaint)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.pointsInPaint], debugDescription: "Value for \"\(RootKeys.pointsInPaint.rawValue)\" needs to be a valid Double"))
        }
        self.pointsInPaint = pointsInPaint
        
        guard let biggestLead = Int(try container.decode(String.self, forKey: .biggestLead)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.biggestLead], debugDescription: "Value for \"\(RootKeys.biggestLead.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let secondChancePoints = Int(try container.decode(String.self, forKey: .secondChancePoints)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.secondChancePoints], debugDescription: "Value for \"\(RootKeys.secondChancePoints.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let pointsOffTurnovers = Int(try container.decode(String.self, forKey: .pointsOffTurnovers)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.pointsOffTurnovers], debugDescription: "Value for \"\(RootKeys.pointsOffTurnovers.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let longestRun = Int(try container.decode(String.self, forKey: .longestRun)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.longestRun], debugDescription: "Value for \"\(RootKeys.longestRun.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let pointsInt = Int(try totalsContainer.decode(String.self, forKey: .points)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: totalsContainer.codingPath + [RootKeys.TotalsKeys.points], debugDescription: "Value for \"\(RootKeys.TotalsKeys.points.rawValue)\" needs to be a valid Double"))
        }
        self.points = pointsInt
        
        // Assists
        guard let assists = Int(try totalsContainer.decode(String.self, forKey: .assists)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: totalsContainer.codingPath + [RootKeys.TotalsKeys.assists], debugDescription: "Value for \"\(RootKeys.TotalsKeys.assists.rawValue)\" needs to be a valid Double"))
        }
        self.assists = assists
        
        // Total Rebounds
        guard let totReb = Int(try totalsContainer.decode(String.self, forKey: .totReb)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: totalsContainer.codingPath + [RootKeys.TotalsKeys.totReb], debugDescription: "Value for \"\(RootKeys.TotalsKeys.totReb.rawValue)\" needs to be a valid Double"))
        }
        self.totReb = totReb
        
        // Blocks
        guard let blocks = Int(try totalsContainer.decode(String.self, forKey: .blocks)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: totalsContainer.codingPath + [RootKeys.TotalsKeys.blocks], debugDescription: "Value for \"\(RootKeys.TotalsKeys.blocks.rawValue)\" needs to be a valid Double"))
        }
        self.blocks = blocks
        
        // Steals
        guard let steals = Int(try totalsContainer.decode(String.self, forKey: .steals)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.steals], debugDescription: "Value for \"\(RootKeys.TotalsKeys.steals.rawValue)\" needs to be a valid Double"))
        }
        self.steals = steals
        
        // Turnovers
        guard let turnovers = Int(try totalsContainer.decode(String.self, forKey: .turnovers)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.turnovers], debugDescription: "Value for \"\(RootKeys.TotalsKeys.turnovers.rawValue)\" needs to be a valid Double"))
        }
        self.turnovers = turnovers
        
        // Personal fouls
        guard let pFouls = Int(try totalsContainer.decode(String.self, forKey: .pFouls)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.pFouls], debugDescription: "Value for \"\(RootKeys.TotalsKeys.pFouls.rawValue)\" needs to be a valid Double"))
        }
        self.pFouls = pFouls
        
        // Team fouls
        guard let teamFouls = Int(try totalsContainer.decode(String.self, forKey: .teamFouls)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.teamFouls], debugDescription: "Value for \"\(RootKeys.TotalsKeys.teamFouls.rawValue)\" needs to be a valid Double"))
        }
        self.teamFouls = teamFouls
        
        // MARK: Field goal stats
        guard let fgm = Int(try totalsContainer.decode(String.self, forKey: .fgm)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.fgm], debugDescription: "Value for \"\(RootKeys.TotalsKeys.fgm.rawValue)\" needs to be a valid Double"))
        }
        self.fgm = fgm
        
        guard let fga = Int(try totalsContainer.decode(String.self, forKey: .fga)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.fga], debugDescription: "Value for \"\(RootKeys.TotalsKeys.fga.rawValue)\" needs to be a valid Double"))
        }
        self.fga = fga
        ...
        guard let ftm = Int(try totalsContainer.decode(String.self, forKey: .ftm)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.ftm], debugDescription: "Value for \"\(RootKeys.TotalsKeys.ftm.rawValue)\" needs to be a valid Double"))
        }
        self.ftm = ftm
        
        guard let fta = Int(try totalsContainer.decode(String.self, forKey: .fta)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.fta], debugDescription: "Value for \"\(RootKeys.TotalsKeys.fta.rawValue)\" needs to be a valid Double"))
        }
        self.fta = fta
        ...
        guard let tpm = Int(try totalsContainer.decode(String.self, forKey: .tpm)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.tpm], debugDescription: "Value for \"\(RootKeys.TotalsKeys.tpm.rawValue)\" needs to be a valid Double"))
        }
        self.tpm = tpm
        
        guard let tpa = Int(try totalsContainer.decode(String.self, forKey: .tpa)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.tpa], debugDescription: "Value for \"\(RootKeys.TotalsKeys.tpa.rawValue)\" needs to be a valid Double"))
        }
        self.tpa = tpa
        ...
        guard let offReb = Int(try totalsContainer.decode(String.self, forKey: .offReb)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.offReb], debugDescription: "Value for \"\(RootKeys.TotalsKeys.offReb.rawValue)\" needs to be a valid Double"))
        }
        self.offReb = offReb
        
        guard let defReb = Int(try totalsContainer.decode(String.self, forKey: .defReb)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.defReb], debugDescription: "Value for \"\(RootKeys.TotalsKeys.defReb.rawValue)\" needs to be a valid Double"))
        }
        self.defReb = defReb
        ...
        guard let shortTimeoutRemaining = Int(try totalsContainer.decode(String.self, forKey: .shortTimeoutRemaining)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.shortTimeoutRemaining], debugDescription: "Value for \"\(RootKeys.TotalsKeys.shortTimeoutRemaining.rawValue)\" needs to be a valid Double"))
        }
        ...
        guard let fullTimeoutRemaining = Int(try totalsContainer.decode(String.self, forKey: .fullTimeoutRemaining)) else {
            throw DecodingError.typeMismatch(Int.self, DecodingError.Context(codingPath: container.codingPath + [RootKeys.TotalsKeys.fullTimeoutRemaining], debugDescription: "Value for \"\(RootKeys.TotalsKeys.fullTimeoutRemaining.rawValue)\" needs to be a valid Double"))
        }
